...
public final class CaseInformationRequestValidation {
    ...
    private CaseInformationRequestValidation() {
    }
    ...
    public static ValidationResult validate(
        ...
        if (requestedInformation == null || requestedInformation.isBlank()) {
            return ValidationResult.invalid("Requested information must not be empty.");
        }
        String trimmedName = engineerName.strip();
        String trimmedEmail = engineerEmail.strip();
        String trimmedText = requestedInformation.strip();
        ...
        if (trimmedText.length() > MAX_REQUESTED_INFORMATION_LENGTH) {
            return ValidationResult.invalid(
                "Requested information exceeds maximum length of " + MAX_REQUESTED_INFORMATION_LENGTH + " characters."
            );
        }
        if (engineerPhone != null && !engineerPhone.isBlank()
            && engineerPhone.strip().length() > MAX_ENGINEER_PHONE_LENGTH) {
            return ValidationResult.invalid(
